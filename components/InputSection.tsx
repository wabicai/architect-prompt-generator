...
export const InputSection: React.FC<InputSectionProps> = ({
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      ...
      <div
        className={`w-full lg:w-80 h-32 lg:h-auto relative flex flex-col rounded-xl transition-all duration-300 overflow-hidden
          ${currentImage
        ...
        {currentImage ? (
          <>
             ...
             <div className="absolute top-2 right-2">
                <button
                  onClick={onClear}
                  className="bg-black/50 hover:bg-black/70 text-white p-1.5 rounded-lg backdrop-blur-md transition-colors"
                  title="Remove Image"
                >
          ...
          <div
            className="flex-1 flex flex-col items-center justify-center text-center p-4 cursor-pointer"
          ...
          >
            ...
            <div className={`p-3 rounded-full mb-2 transition-transform duration-300 ${isDragging ? 'scale-110 bg-brand-100 text-brand-600' : 'bg-white shadow-sm text-slate-400'}`}>
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
            </div>
            <p className="text-xs text-slate-500">
              Drop screenshot or <span className="text-brand-600 font-medium">click to upload</span>
            </p>
            <p className="text-xs text-slate-400 mt-0.5">Paste with Ctrl+V</p>
          </div>
        )}
      </div>
    </div>
